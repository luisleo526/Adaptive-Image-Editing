# ref: https://github.com/EPFL-VILAB/3DCommonCorruptions
...
def separable_gaussian(img, r=3.5, cutoff=None, device=None):
    ...
    r_pad_half = r_pad // 2
    # print(r_pad_half)
    img = F.pad(img, (r_pad_half, r_pad_half, r_pad_half, r_pad_half),
                "replicate", 0)  # effectively zero padding
    filtered = F.conv2d(img, fil.unsqueeze(
        1).unsqueeze(-2), bias=None, stride=1, padding=0, dilation=1, groups=n_channels)
    filtered /= filsum
    filtered = F.conv2d(filtered, fil.unsqueeze(
        1).unsqueeze(-1), bias=None, stride=1, padding=0, dilation=1, groups=n_channels)
    filtered /= filsum
...
def compute_circle_of_confusion(depths, aperture_size, focal_length, focus_distance):
    ...
    c = aperture_size * torch.abs(depths - focus_distance) / \
        depths * (focal_length / (focus_distance - focal_length))
...
def compute_quantile_membership(depth, quantile_vals):
    ...
    quantile_right = torch.gather(
        quantile_vals_unsqueezed, 1, calculated_quantiles).reshape(depth.shape)
    quantile_left = torch.gather(
        quantile_vals_unsqueezed, 1, calculated_quantiles_left).reshape(depth.shape)
    quantile_dists = quantile_right - quantile_left
    # / quantile_dists[calculated_quantiles_left]
    dist_right = ((quantile_right - depth) / quantile_dists)
    # / quantile_dists[calculated_quantiles_left]
    dist_left = ((depth - quantile_left) / quantile_dists)
...
def get_blur_stack_single_image(rgb, blur_radii, cutoff_multiplier):
    ...
    for r in blur_radii:
        cutoff = None if cutoff_multiplier is None else int(
            r * cutoff_multiplier)
...
def get_blur_stack(rgb, blur_radii, cutoff_multiplier=None):
    ...
    args = [(image.unsqueeze(0), radii, cutoff_multiplier)
            for image, radii in zip(rgb, blur_radii)]
...
def composite_blur_stack(blur_stack, dist_left, dist_right, values_left, values_right):
    ...
    shape[2] = 1
    composite_vals = torch.zeros(
        shape, dtype=torch.float32, device=blur_stack.device)
    sim_left = (1 - dist_left**2)
    sim_right = (1 - dist_right**2)

    _ = composite_vals.scatter_(1, index=values_left.unsqueeze(
        1).unsqueeze(2), src=sim_left.unsqueeze(1).unsqueeze(2))
    _ = composite_vals.scatter_(1, index=values_right.unsqueeze(
        1).unsqueeze(2), src=sim_right.unsqueeze(1).unsqueeze(2))
...
def refocus_image(rgb, depth, focus_distance, aperture_size, quantile_vals, return_segments=False):
    ...
    dist_left, dist_right, calculated_quantiles_left, calculated_quantiles = compute_quantile_membership(
        depth, quantile_vals)
    blur_radii = compute_circle_of_confusion_no_magnification(
        quantile_vals, aperture_size, focus_distance)
    # print(blur_radii)
    blur_stack = get_blur_stack(rgb, blur_radii, cutoff_multiplier=3)
    composited = composite_blur_stack(
        blur_stack, dist_left, dist_right, calculated_quantiles_left, calculated_quantiles)
...
def defocus_blur_3D(rgb_batch,
    ...
    def refocus_image_(rgb, depth, focus_idxs, i, return_segments=False):
        ...
        with torch.no_grad():
            ...
            quantiles = torch.arange(
                0, n_quantiles + 1, device=device) / n_quantiles
            depth_normalized = depth

            quantiles, quantile_vals = compute_quantiles(
                depth_normalized, quantiles, eps=0.0001)
            quantile_vals = quantile_vals.permute(1, 0)

            aperture = severity * 2 + 1  # hard-coded!!

            focus_dist_idxs = sample_idxs_2(quantiles).cuda()
            focus_idxs[i] = focus_dist_idxs
            focus_dists = torch.gather(
                quantile_vals, 1, focus_dist_idxs.unsqueeze(0)).permute(1, 0)

            copies_to_return = 2
            apertures = torch.tensor(
                [[aperture]] * copies_to_return, dtype=torch.float32, device=device)
            # apertures[1] = (apertures[1] - 2.5) / 2. #reduce aperture for far focus
            apertures[1] = (apertures[1] - 2.5) / \
                1.  # reduce aperture for far focus

            return refocus_image(rgb, depth_normalized, focus_dists, apertures, quantile_vals, return_segments)

    batch_size = len(rgb_batch)

    rgb_batch, depth_batch = replicate_batch(
        rgb_batch), replicate_batch(depth_batch)
    ...
    args = [(rgb_batch[i], depth_batch[i], focus_idxs, i)
...
def create_dof_data(BASE_PATH_RGB=None, BASE_PATH_DEPTH=None, BASE_TARGET_PATH=None, BATCH_SIZE=1):
    ...
    for corruption in corruptions_to_generate:
        for severity in range(1, 4):
            TARGET_PATH = os.path.join(
                BASE_TARGET_PATH, corruption, str(severity), CLASS)
    ...
    save_corrupted_batches(rgb_and_depth_loader,
                           save_path=BASE_TARGET_PATH, CLASS=CLASS)
