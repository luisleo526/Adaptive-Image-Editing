def process_load(npyImage, objSettings):
    ...
    tenDepth = (objCommon['fltFocal'] *
                objCommon['fltBaseline']) / (tenDisparity + 0.0000001)
    ...
    objCommon['objDepthrange'] = cv2.minMaxLoc(
        src=tenDepth[0, 0, 20:-20, 20:-20].detach().cpu().numpy(), mask=None)
...
def process_inpaint(tenShift):
    ...
    objInpainted['tenDepth'] = (
        objCommon['fltFocal'] * objCommon['fltBaseline']) / (objInpainted['tenDisparity'] + 0.0000001)
    objInpainted['tenValid'] = (spatial_filter(
        objInpainted['tenDisparity'] / objInpainted['tenDisparity'].max(), 'laplacian').abs() < 0.03).float()
    ...
    objCommon['tenInpaImage'] = torch.cat([objCommon['tenInpaImage'], objInpainted['tenImage'].view(
        1, 3, -1)[tenMask.expand(-1, 3, -1)].view(1, 3, -1)], 2)
    objCommon['tenInpaDisparity'] = torch.cat([objCommon['tenInpaDisparity'], objInpainted['tenDisparity'].view(
        1, 1, -1)[tenMask.expand(-1, 1, -1)].view(1, 1, -1)], 2)
    objCommon['tenInpaDepth'] = torch.cat([objCommon['tenInpaDepth'], objInpainted['tenDepth'].view(
        1, 1, -1)[tenMask.expand(-1, 1, -1)].view(1, 1, -1)], 2)
    objCommon['tenInpaPoints'] = torch.cat([objCommon['tenInpaPoints'], objInpainted['tenPoints'].view(
        1, 3, -1)[tenMask.expand(-1, 3, -1)].view(1, 3, -1)], 2)
# end
...
def process_shift(objSettings):
    ...
    fltClosestFromX = (
        (fltClosestFromU - (objCommon['intWidth'] / 2.0)) * fltClosestDepth) / objCommon['fltFocal']
    fltClosestFromY = (
        (fltClosestFromV - (objCommon['intHeight'] / 2.0)) * fltClosestDepth) / objCommon['fltFocal']
    fltClosestToX = (
        (fltClosestToU - (objCommon['intWidth'] / 2.0)) * fltClosestDepth) / objCommon['fltFocal']
    fltClosestToY = (
        (fltClosestToV - (objCommon['intHeight'] / 2.0)) * fltClosestDepth) / objCommon['fltFocal']
    ...
    tenShift = torch.FloatTensor(
        [fltShiftX, fltShiftY, fltShiftZ]).view(1, 3, 1).cuda()
...
def process_autozoom(objSettings):
    npyShiftU = numpy.linspace(-objSettings['fltShift'],
                               objSettings['fltShift'], 16)[None, :].repeat(16, 0)
    npyShiftV = numpy.linspace(-objSettings['fltShift'],
                               objSettings['fltShift'], 16)[:, None].repeat(16, 1)
    fltCropWidth = objSettings['objFrom']['intCropWidth'] / \
        objSettings['fltZoom']
    fltCropHeight = objSettings['objFrom']['intCropHeight'] / \
        objSettings['fltZoom']
    ...
    for intU in range(16):
        for intV in range(16):
            ...
            tenRender, tenExisting = render_pointcloud(tenPoints, objCommon['tenRawImage'].view(
                1, 3, -1), objCommon['intWidth'], objCommon['intHeight'], objCommon['fltFocal'], objCommon['fltBaseline'])
...
def process_kenburns(objSettings):
    ...
    if 'boolInpaint' not in objSettings or objSettings['boolInpaint'] == True:
        objCommon['tenInpaImage'] = objCommon['tenRawImage'].view(1, 3, -1)
        objCommon['tenInpaDisparity'] = objCommon['tenRawDisparity'].view(
            1, 1, -1)
        objCommon['tenInpaDepth'] = objCommon['tenRawDepth'].view(1, 1, -1)
        objCommon['tenInpaPoints'] = objCommon['tenRawPoints'].view(1, 3, -1)

        for fltStep in [0.0, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.0]:
            fltFrom = 1.0 - fltStep
            fltTo = 1.0 - fltFrom
            ...
            fltDepthFrom = objCommon['objDepthrange'][0]
            fltDepthTo = objCommon['objDepthrange'][0] * (fltCropWidth / max(
                objSettings['objFrom']['intCropWidth'], objSettings['objTo']['intCropWidth']))
    ...
    for fltStep in objSettings['fltSteps']:
        ...
        tenRender, tenExisting = render_pointcloud(tenPoints, torch.cat([objCommon['tenInpaImage'], objCommon['tenInpaDepth']], 1).view(
            1, 4, -1), objCommon['intWidth'], objCommon['intHeight'], objCommon['fltFocal'], objCommon['fltBaseline'])

        tenRender = fill_disocclusion(
            tenRender, tenRender[:, 3:4, :, :] * (tenExisting > 0.0).float())

        npyOutput = (tenRender[0, 0:3, :, :].detach().cpu().numpy().transpose(
            1, 2, 0) * 255.0).clip(0.0, 255.0).astype(numpy.uint8)
        npyOutput = cv2.getRectSubPix(image=npyOutput, patchSize=(max(objSettings['objFrom']['intCropWidth'], objSettings['objTo']['intCropWidth']), max(
            objSettings['objFrom']['intCropHeight'], objSettings['objTo']['intCropHeight'])), center=(objCommon['intWidth'] / 2.0, objCommon['intHeight'] / 2.0))
        npyOutput = cv2.resize(src=npyOutput, dsize=(
            objCommon['intWidth'], objCommon['intHeight']), fx=0.0, fy=0.0, interpolation=cv2.INTER_LINEAR)
...
def preprocess_kernel(strKernel, objVariables):
    ...
    while True:
        ...
        strKernel = strKernel.replace(objMatch.group(), str(intStrides[intArg] if torch.is_tensor(
            intStrides[intArg]) == False else intStrides[intArg].item()))
    # end
...
def launch_kernel(strFunction, strKernel):
    if 'CUDA_HOME' not in os.environ:
        os.environ['CUDA_HOME'] = sorted(
            glob.glob('/usr/lib/cuda*') + glob.glob('/usr/local/cuda*'))[-1]
    # end
...
def depth_to_points(tenDepth, fltFocal):
    tenHorizontal = torch.linspace((-0.5 * tenDepth.shape[3]) + 0.5, (0.5 * tenDepth.shape[3]) - 0.5, tenDepth.shape[3]).view(
        1, 1, 1, -1).expand(tenDepth.shape[0], -1, tenDepth.shape[2], -1)
    tenHorizontal = tenHorizontal * (1.0 / fltFocal)
    tenHorizontal = tenHorizontal.type_as(tenDepth)

    tenVertical = torch.linspace((-0.5 * tenDepth.shape[2]) + 0.5, (0.5 * tenDepth.shape[2]) - 0.5, tenDepth.shape[2]).view(
        1, 1, -1, 1).expand(tenDepth.shape[0], -1, -1, tenDepth.shape[3])
...
def spatial_filter(tenInput, strType):
    ...
    if strType == 'laplacian':
        tenLaplacian = tenInput.new_zeros(
            tenInput.shape[1], tenInput.shape[1], 3, 3)
        ...
        tenOutput = torch.nn.functional.conv2d(
            input=tenOutput, weight=tenLaplacian)
        ...
        tenOutput = tenOutput.contiguous().view(
            tenOutput.shape[0], tenOutput.shape[1], tenOutput.shape[2], tenOutput.shape[3], 3 * 3)
        ...
        tenOutput = tenOutput.contiguous().view(
            tenOutput.shape[0], tenOutput.shape[1], tenOutput.shape[2], tenOutput.shape[3], 5 * 5)
        tenOutput = tenOutput.median(-1, False)[0]
...
def render_pointcloud(tenInput, tenData, intWidth, intHeight, fltFocal, fltBaseline):
    tenData = torch.cat([tenData, tenData.new_ones(
        [tenData.shape[0], 1, tenData.shape[2]])], 1)

    tenZee = tenInput.new_zeros(
        [tenData.shape[0], 1, intHeight, intWidth]).fill_(1000000.0)
	...
	''', {
        ...
        block=tuple([512, 1, 1]),
        args=[cupy.int32(n), tenInput.data_ptr(),
	...
	''', {
        ...
        grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
        block=tuple([512, 1, 1]),
        args=[cupy.int32(n), tenInput.data_ptr(), tenData.data_ptr(),
              tenZee.data_ptr(), tenOutput.data_ptr()]
    )
...
def fill_disocclusion(tenInput, tenDepth):
	...
	''', {
        ...
        block=tuple([512, 1, 1]),
        args=[cupy.int32(n), tenInput.data_ptr(),
              tenDepth.data_ptr(), tenOutput.data_ptr()]
    )
